...
\begin{frame}{Sección Medio Punto}{Ejercicio 14}
    ...
    \begin{enumerate}[I.]
        ...
        \item $\lreg \subset \lcf \Rightarrow L \in \lcd \Rightarrow$ Cumple el Lema de Bombeo para los Lenguajes libres de contexto.
...
\begin{frame}{Sección Medio Punto}{Ejercicio 14}
    ...
    \vspace{3mm}
    Sea $M_{\emptyset}$ la máquina de Turing que acepta el lenguaje vacío. Dada una MT $M \in EMPTY^{MT}$, tenemos que $\langle M, M_{\emptyset}\rangle \in EQ^{MT}$. Por tanto, $EMPTY^{MT} \leq_{m} EQ^{MT}$. Pero esto implicaría que $EMPTY^{MT}$ es $\lr$ por el 2ndo Teorema de Reducibilidad. CONTRADICCIÓN.
...
\begin{frame}{Sección Medio Punto}{Ejercicio 14}
    ...
    Ante una entrada w, M' actúa así:
    \begin{enumerate}[1.] 
        \item Ejecuta paralelamente $M_1$ y $M_2$ con w. 
        \item   Si $M_1$ ACEPTA, M' ACEPTA.\\
                Si $M_2$ ACEPTA, M' RECHAZA.
    \end{enumerate}
        
    \vspace{3mm}
    Sabemos que es proceso terminará en tiempo finito, pues o bien $w \in L$ o $w \notin L$.
